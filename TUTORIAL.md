...
invoice. Each template should work on all invoices for a company or
...
All the regex `fields` you need extracted. Required fields, by default, are `amount`,
...
single value, or an array (depending on an amount of unique matches found).
...
`invoice2data invoice-XXX.pdf --template-folder tpl --debug`
